...
// ── Lottie segments (read-only from constants) ─────────────────────────────
const SEG_FULL = [60, 240];
const SEG_IDLE = [67, 89];

// ── Typewriter hook (local copy to avoid import path coupling) ─────────────
...
const SLIDES = [
  {
    text: 'Hola, soy Redo. Tu asistente de estudio personal.',
    segment: 'full',
  },
  {
    // text is dynamic — built from weather
    text: null,
    segment: 'idle',
  },
  {
  ...
  {
    text: 'Arrastrame por la app. Puedo hablarte de cualquier materia que toque.',
    segment: 'idle',
    showDragDemo: true,
  },
  {
    text: 'Estoy en versión beta. Puedo equivocarme, pero aprendo con vos. Reportá cualquier error.',
    segment: 'idle',
    showBeta: true,
  },
  {
...
export default function CinematicOnboarding({ user, onComplete }) {
  ...
  // ── Build slide text (weather-aware for slide 1) ─────────────────────────
  ...
  // Idle ping-pong on loop complete
  useEffect(() => {
    const l = lottieRef.current;
    if (!l) return;
    const onComplete = () => {
      if (SLIDES[step].segment === 'idle') {
        idleDirRef.current *= -1;
        l.setDirection(idleDirRef.current);
        l.playSegments(SEG_IDLE, true);
      }
    };
    l.addEventListener('complete', onComplete);
    return () => l.removeEventListener('complete', onComplete);
  }, [step]);
  ...
  // ── Handle bubble tap (skip typewriter, or advance if complete) ──────────
  ...
  return (
    <div style={{
      ...
      {/* ── Progress dots ─────────────────────────────────────────────── */}
      <div style={{
        display: 'flex', gap: '6px', width: '100%', maxWidth: '280px',
      }}>
        {SLIDES.map((_, i) => (
          <div key={i} style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          height: 120,
          animation: step === 0
            ? 'onboarding-zoom-in 0.6s ease-out both'
            : undefined,
          filter: 'drop-shadow(0 0 16px rgba(120, 180, 255, 0.9)) drop-shadow(0 0 32px rgba(120, 160, 255, 0.5))',
          animationName: step > 0 ? 'aurora-pulse' : undefined,
          animationDuration: '1.5s',
          animationTimingFunction: 'ease-in-out',
          animationIterationCount: 'infinite',
          animationDirection: 'alternate',
          ...(slide.showDragDemo ? {
            animation: 'onboarding-drag-demo 2.5s ease-in-out infinite',
          } : {}),
        }}>
          <Lottie
            lottieRef={lottieRef}
      ...
      {/* ── Bottom: Continue button (not on final slide) ──────────────── */}
      {!slide.isFinal && (
        <button
